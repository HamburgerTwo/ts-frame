...
export const http: Middleware<Response> = (_: Context, next: NextFunction<Response>) => {
  return next().then<Response>(
    response => (response.status >= 200 && response.status <= 299 ? response : Promise.reject(response)),
  );
};
