...
import { sendValidateCode, checkValidateCode } from '../../services/index';
    ...
    checkValidateCode({
      ...
      .then(res =>
        bindingPhone({
          ...
          .then(res =>
            findEmployeeById({ memberId: res.memberId || 0 }).catch(err => {
              console.info(err);
            }),
          )
          .catch(err => {
  ...
  render() {
    ...
    return (
      <div className={s.root}>
          ...
          <div className={s.fieldset}>
            ...
            <div className={s.inputBox}>
              <div className={s.inputStyle}>
                <input
                  className={s.innerInput}
                  placeholder="请填写门店名称"
                  value={orgNo}
                  readOnly
                />
                <button disabled className={s.queryBtn}>
                  查询
                </button>
              </div>
              <div className={s.info}>
                查询结果：您当前绑定门店为<span className={s.underline}>{orgName}</span>
              </div>
            </div>
          </div>
        ) : null}
        <div className={s.fieldset}>
          ...
          {isSign ? null : (
            <div className={s.inputBox}>
              <div className={s.inputStyle}>
                <input
                  onChange={this.onValicodeChange}
                  className={s.innerInput}
                  placeholder="请输入验证码"
                  ref={ref => (this.valicode = ref)}
                />
                <button onClick={this.sendValidateCode} className={s.queryBtn}>
        ...
        {isSign ? null : (
          <div className={s.fieldset}>
            <button className={s.confirmBtn} onClick={this.onConfirm}>
              确定
            </button>
          </div>
        )}
...
export default connect(
  ...
  (dispatch: UserAction) => ({
    ...
    findEmployeeById: (params: findEmployeeByIdParam) =>
      dispatch(Actions.findEmployeeByIdAction(params)),
    findOrganizationByIdOrNo: (params: findOrganizationByIdOrNoParam) =>
      dispatch(Actions.findOrganizationByIdOrNoAction(params)),
